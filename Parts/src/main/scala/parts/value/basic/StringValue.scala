...
import parts.value.basic.StringValue.CharToString
...
object StringValue {
    ...
    final case object StringNull extends StringValue {
        override def get(implicit valueContext: ValueContext): Option[String] = None
    }
    ...
    final case class Length(value: StringValue) extends IntValue {
        override def get(implicit valueContext: ValueContext): Option[Int] = value.get.map(v => v.length)
    }
    
    final case class NumericToString(value: NumericValue) extends StringValue {
        override def get(implicit valueContext: ValueContext): Option[String] = value.get match {
    ...
    final case class CharToString(value: CharValue) extends StringValue {
        override def get(implicit valueContext: ValueContext): Option[String] = value.get.map(v => v.toString)
    }
